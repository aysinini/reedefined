...
create policy "Anyone can read submitted contributions"
  on public.contributions for select
...
-- Run this if contributions table already exists
alter table public.contributions add column if not exists tagline text;
alter table public.contributions add column if not exists status text default 'draft';
alter table public.contributions add column if not exists updated_at timestamptz default now();

-- Add unique constraint on user_id so upsert works (one draft per user)
do $$ begin
  if not exists (
    select 1 from pg_constraint where conname = 'contributions_user_id_key'
  ) then
    alter table public.contributions add constraint contributions_user_id_key unique (user_id);
  end if;
end $$;

-- Allow users to update own contributions
